...
Rome: 4 x 4 road tiles over the comune and the near province, plus the metro
tunnels (railway=subway) for lines A, B, B1 and C.
"""
import json, os, re, sys
import osmium

ROOT = os.path.join(os.path.dirname(__file__), '..')
PBFS = [os.path.join(ROOT, 'data', 'centro-latest.osm.pbf')]

# must match pipeline/download.sh
S, N, W, E = 41.61, 42.14, 12.19, 12.84
RAIL_BOX = (41.7, 12.34, 42.0, 12.66)
...
RAIL = re.compile(r'^(subway|light_rail|rail|construction)$')
...
rail_file = os.path.join(ROOT, 'data/osm/rome-rail.json')
...
GEN = 'pbf-tiles.py (Geofabrik italy/centro)'
