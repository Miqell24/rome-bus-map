...
// Rome: ONE feed from Roma Servizi per la Mobilità — 327 Atac lines across the
// comune plus 100 more run by the operators of the near province (Roma TPL's
// successors: TROIANI, BIS, ATR Mobility, TUSCIA), and the four metro lines
// A, B, B1 and C in their official colours.
// Usage: node pipeline/build.mjs [--all | lines...] [--tram all|A,B,C]
...
// Exact-match repairs for poles a feed names badly. Rome's problem is not a
// handful of poles but 8 130 of them — see lib/caps.mjs.
const NAME_FIX = {};
...
// ONE feed, TWO cfgs.
//
// romamobilita.it publishes the whole AVM dataset as one GTFS with shapes.
// route_type 3 is the bus (Atac plus the four provincial operators), 1 the
// metro.
//
// NOT IN THE FEED, and so not on this map: the six tram lines (2, 3, 5, 8, 14,
// 19) and the three ex-concession railways (Roma–Lido, Roma–Viterbo,
// Termini–Centocelle). Atac runs them all, but this bundle carries only its
// buses and the metro — the feed's own gap, not a decision made here. What IS
// in it under a tram-like name is "3L", the bus that stands in for tram 3
// while the track is closed.
//
// STOP NAMES: 8 130 of 8 299 shout ("STAZIONE PONTE MAMMOLO"), and the caps
// lose their accents or write them as a trailing apostrophe (CITTA'). The
// rewrite goes through a dictionary harvested from the OSM extract this build
// already reads — the Athens recipe, retold in Italian, with Roman numerals
// (XX Settembre, IV Novembre) protected because Rome is full of them.
import { buildNameDict, latinTitleCase } from './lib/caps.mjs';

const LBL = new Map();

// The metro keys the feed uses are MEA/MEB/MEB1/MEC; the station signs say A,
// B, B1 and C, and that is what gets printed.
const isRailTrunk = (l) => /^ME/.test(l);
const metroKey = (sn) => {
  const s0 = (sn || '').trim();
  if (!s0) return null;
  const bare = s0.replace(/^ME/, '');
  if (bare !== s0) LBL.set(s0, bare);
  return s0;
};

const CAPS_ACRONYMS = new Set(['MA', 'MB', 'MC', 'FS', 'FL', 'ATAC', 'EUR', 'CNR',
  'ENEA', 'INPS', 'RAI', 'ACEA', 'ASL', 'IBM', 'SNC', 'PZA', 'VLE', 'LGO',
  'CIRC', 'CIMITERO', 'GRA']);
const nameDict = (() => {
  const docs = [];
  for (let i = 1; i <= 16; i++) {
    const p = join(ROOT, `data/osm/tiles/t${i}.json`);
    if (existsSync(p)) docs.push(JSON.parse(readFileSync(p, 'utf8')));
  }
  return buildNameDict(docs);
})();
const romeName = (n) => latinTitleCase(n.replace(/\s+/g, ' ').trim(), nameDict, CAPS_ACRONYMS);

const MODES = [{
  mode: 'bus', label: 'buses', graphMode: 'road',
  // the comune and the near province — 4 × 4 tiles
...
if (tramAll || tramSel.length) MODES.push({
  mode: 'tram', label: 'metro', osmFile: 'data/osm/rome-rail.json',
  graphMode: 'tram', railKeep: new Set(['subway']),
  // line B runs in the open from Ponte Mammolo out to Rebibbia and line C's
  // outer half is on the old Termini–Pantano alignment
  railExtra: (e) => e.tags?.railway === 'light_rail',
  color: '#d6212b', colorDark: '#7c1116',
  all: tramAll, lines: tramAll ? [] : tramSel.map((l) => (/^ME/.test(l) ? l : 'ME' + String(l).toUpperCase())),
  feeds: [
    { tag: 'rm', dir: 'data/gtfs', routeTypes: ['1'], mapKey: metroKey, nameFix: romeName },
  ],
});
...
await (await import('./night.mjs')).nightPass(outDir, /^n/, { sort: false });
